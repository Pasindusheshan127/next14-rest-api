...
export const GET = async (request: Request) => {
  try {
    ...
    return new NextResponse("Error in getting blog:" + error.massage, {
      status: 500,
    });
  }
};
...
export const POST = async (request: Request) => {
  try {
    ...
    await newBlog.save();
    return new NextResponse(JSON.stringify("blog is created" + newBlog), {
      status: 200,
    });
  } catch (error: any) {
    return new NextResponse("Error in creating blog" + error.massage, {
      status: 500,
    });
  }
};
